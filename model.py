...
#         game_data = play_game_record_moves(board_with_ships) # random board states
...
#     return np.array(boards, dtype=np.float32), np.array(labels, dtype=np.float32)
...
for epoch in tqdm(range(num_epochs)):  # loop over the dataset multiple times
    ...
    for i, data in enumerate(tqdm(train_dataloader, 0)):
        ...
        outputs = net(inputs.unsqueeze(1)) # channel dimension
